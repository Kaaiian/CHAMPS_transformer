...
class CrabNet(nn.Module):
    ...
    def __init__(self,
        ...
        # self.fc1 = nn.Linear(19, self.d_model)  # uncomment for "molecule vector learning"
        self.fc2 = nn.Linear(self.d_model, self.d_model)
        ...
        self.output_nn = ResidualNetwork(self.d_model, self.output_dims, out_hidden)
        self.leaky = nn.LeakyReLU()
    def forward(self, x, mol_id):
        """
        ...
        """
        # x = x[:, :, -19:]  # uncomment for "molecule vector learning" (baseline)
        # print(x.shape)
        x = self.fc1(x)
        x = self.leaky(x)
        x = self.dropout1(x)
